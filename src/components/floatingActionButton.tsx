...
export default function FloatingActionButton({
  ...
  handleBottomPress: (item: string) => void;
}) {
  ...
  const handleBPress = (item: string) => {
    handleBottomPress(item);
    handlePress();
  };
  ...
  return (
    <View style={styles.container}>
      <Animated.View
        ...
        onTouchStart={() => handleBPress("Learned")}
      >
        <View style={styles.iconContainer}>
          <Ionicons name="rocket-outline" size={28} color="black" />
        </View>
      ...
      <Animated.View
        onTouchStart={() => handleBPress("Review")}
        style={[styles.contentContainer, secondIcon, secondWidthStyle]}
      >
        <View style={styles.iconContainer}>
          <MaterialCommunityIcons name="bug-outline" size={28} color="black" />
        </View>
        <Animated.Text style={[styles.text, opacityText]}>Review</Animated.Text>
      </Animated.View>

      <Animated.View
        onTouchStart={() => handleBPress("Saved")}
        style={[styles.contentContainer, firstIcon, firstWidthStyle]}
      >
        <View style={styles.iconContainer}>
          <Ionicons name="bookmark-outline" size={30} color="black" />
        </View>
        <Animated.Text style={[styles.text, opacityText]}>Saved</Animated.Text>
      </Animated.View>
      <Pressable
